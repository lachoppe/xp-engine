...
bool vkutil::LoadImageFromAsset(VulkanEngine& engine, const char* filepath, AllocatedImage& outImage)
{
	...
	if (!loaded)
	{
		OutputMessage("Error loading image: %s", filepath);
		return false;
	}
	...
	VkDeviceSize imageSize = info.dataSize;
	VkFormat imageFmt{};
	switch (info.textureFormat)
	{
	case assets::TextureFormat::RGBA8:
		imageFmt = VK_FORMAT_R8G8B8A8_UNORM;
		break;
	default:
		return false;
	}
...
// 	VK_MEMORY_PROPERTY_HOST_CACHED_BIT
	AllocatedBuffer stagingBuffer = engine.CreateBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);

	void* data;
	vmaMapMemory(engine.allocator, stagingBuffer.allocation, &data);
	assets::UnpackTexture(&info, asset.blob.data(), imageSize, reinterpret_cast<char*>(data));
	vmaUnmapMemory(engine.allocator, stagingBuffer.allocation);

	outImage = UploadImage(info.resolution[0], info.resolution[1], imageFmt, engine, stagingBuffer);
