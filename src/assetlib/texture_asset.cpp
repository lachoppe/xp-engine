...
assets::TextureInfo assets::ReadTextureInfo(AssetFile* file)
{
	...
	info.textureFormat = ParseFormat(formatStr.c_str());
...
assets::AssetFile assets::PackTexture(TextureInfo* info, void* pixelData)
{
	...
	{
		ASSERT_M(0, "Invalid texture file compression mode: '%s'\nSource path: %s", compressModeName, info->sourceFile);
	}

	return file;
}

assets::TextureFormat assets::ParseFormat(const char* string)
{
