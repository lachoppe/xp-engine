...
assets::MeshInfo assets::ReadMeshInfo(AssetFile* file)
{
	...
	info.vertexFormat = assets::ParseFormat(formatStr.c_str());
...
assets::AssetFile assets::PackMesh(MeshInfo* info, void* vertexData, void* indexData)
{
	...
	{
		ASSERT_M(0, "Invalid mesh file compression mode: '%s'\nSource path: %s", compressModeName, info->sourceFile);
	}

	return file;
}

assets::VertexFormat assets::ParseFormat(const char* string)
{
